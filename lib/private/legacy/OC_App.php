	...
	public static function loadApp(string $app) {
		...
		if (!empty($info['settings']['admin'])) {
			if (array_key_exists('@value', $info['settings']['admin'])) {
				\OC::$server->get(ISettingsManager::class)->registerSetting('admin', $info['settings']['admin']['@value'], filter_var($info['settings']['admin']['@attributes']['allow-privilege-delegation'], FILTER_VALIDATE_BOOLEAN));
			} else {
				foreach ($info['settings']['admin'] as $setting) {
					if (is_array($setting)) {
						\OC::$server->get(ISettingsManager::class)->registerSetting('admin', $setting['@value'], filter_var($setting['@attributes']['allow-privilege-delegation'], FILTER_VALIDATE_BOOLEAN));
					} else {
